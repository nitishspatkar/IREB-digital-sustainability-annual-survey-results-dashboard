...
const extractOrganizationReportsOnSustainabilityByRoleData: DataExtractor<HorizontalBarData> = (
  responses
) => {
  ...
  responses.forEach((r) => {
    ...
    const reports = norm(r.raw.organizationReportsOnSustainability ?? '');

    if (!role || role.toLowerCase() === 'n/a') {
      return;
    }

    if (reports !== 'yes' && reports !== 'no' && reports !== 'not sure') {
      return;
    }
    ...
    if (reports === 'yes') counts.yes++;
    else if (reports === 'no') counts.no++;
    else counts.notSure++;
  });
...
// --- The Processor Logic ---
const processOrganizationReportsOnSustainabilityByRole: ChartProcessor = (responses, palette) => {
  ...
  responses.forEach((r) => {
    ...
    let label = '';
    if (reports === 'yes') label = 'Yes';
    else if (reports === 'no') label = 'No';
    else label = 'Not sure';
  ...
  // 4. Build Traces
  const traces: Data[] = REPORT_TEMPLATE.map((def) => {
...
// --- The Component ---
export const OrganizationReportsOnSustainabilityByRole = ({
  onBack,
  showBackButton = true,
}: {
  onBack: () => void;
  ...
  return (
    <GenericChart
      graphId="OrganizationReportsOnSustainabilityByRole"
      processor={processOrganizationReportsOnSustainabilityByRole}
      layout={layout}
      isEmbedded={true}
      onBack={showBackButton ? onBack : undefined}
      dataExtractor={extractOrganizationReportsOnSustainabilityByRoleData}
