...
const extractNoTrainingReasonsByAgeData: DataExtractor<HorizontalBarData> = (responses) => {
  const ageCounts = new Map<string, Map<string, number>>();
  ...
  responses.forEach((r) => {
    ...
    if (hasValidNoTrainingReasonAnswer(r)) {
      ...
      if (!ageCounts.has(ageGroup)) {
        ageCounts.set(ageGroup, new Map());
      }
      const reasons = ageCounts.get(ageGroup)!;
...
export const NoTrainingReasonsByAge = ({
  ...
  return (
    <GenericChart
      ...
      dataExtractor={extractNoTrainingReasonsByAgeData}
