...
const extractOrganizationHasSustainabilityTeamByOrgTypeData: DataExtractor<HorizontalBarData> = (
  responses
) => {
...
// --- The Processor Logic ---
const processOrganizationHasSustainabilityTeamByOrgType: ChartProcessor = (responses, palette) => {
  ...
  responses.forEach((r) => {
    ...
    let label = '';
    if (hasTeam === 'yes') label = 'Yes';
    else if (hasTeam === 'no') label = 'No';
    else label = 'Not sure';
  ...
  // 4. Build Traces
  const traces: Data[] = TEAM_TEMPLATE.map((def) => {
...
// --- The Component ---
export const OrganizationHasSustainabilityTeamByOrgType = ({
  onBack,
  showBackButton = true,
}: {
  onBack: () => void;
  ...
  return (
    <GenericChart
      graphId="OrganizationHasSustainabilityTeamByOrgType"
      processor={processOrganizationHasSustainabilityTeamByOrgType}
      layout={layout}
      isEmbedded={true}
      onBack={showBackButton ? onBack : undefined}
      dataExtractor={extractOrganizationHasSustainabilityTeamByOrgTypeData}
